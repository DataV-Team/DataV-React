...
var PercentPond = function PercentPond(_ref) {
  ...
  function update() {
    ...
    if (!config) {
      setState(function (state) {
        return __chunk_3._extends({}, state, { width: width, height: height });
      });
    ...
    setState({
      width: width,
      height: height,
      mergedConfig: __chunk_6.util_2(__chunk_6.util_1(defaultConfig, true), config || {})
    });
  }
  ...
  var classNames = React.useMemo(function () {
    return __chunk_5.classnames('dv-percent-pond', className);
  }, [className]);

...
function getPoints() {
  ...
  return points.reduce(function (all, item) {
    return [].concat(__chunk_3.toConsumableArray(all), __chunk_3.toConsumableArray(item));
  }, []);
}
...
function getData() {
  ...
  var heights = new Array(rowNum * rowPoints).fill(0).map(function (foo) {
    return Math.random() > 0.8 ? __chunk_2.randomExtend(0.7 * h, h) : __chunk_2.randomExtend(0.2 * h, 0.5 * h);
  });
...
var Decoration = function Decoration(_ref) {
  ...
  function calcSVGData() {
    return __chunk_3._extends({}, getData(), {
      points: getPoints(),
  ...
  var classNames = React.useMemo(function () {
    return __chunk_5.classnames('dv-decoration-6', className);
  }, [[className]]);
